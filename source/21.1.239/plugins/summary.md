...
2. You can tag plugins with a "Plugin Environment" which can limit which environment a plugin is valid for (based on the Environment Label).  If unspecified, the plugin will load in any environment.  Environment Labels are the little badge next to the "Q" icon (PROD environments are usually blank) >> ![envlabel](env.png)
